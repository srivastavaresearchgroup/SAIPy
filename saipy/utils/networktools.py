...
def network_detect(output_by_station, path_dat, station_list, dir_stat_name,
                 channel='', format='', min_stat=3, delta_t=3.0, time_and_pattern=True,
                 save_result=True, path_out=''):
    """
    ...
    - min_stat: minimum number of stations required per event (by default 3).
    ...
    - time_and_pattern (bool): Whether to apply time_difference & pattern clustering.
        For Simple time_difference clustering set to False (default: True).
    ...
    for date_key, list_detections in all_detect_ev:
        ...
        if time_and_pattern:
            ...
            clusters = cluster_detections(dict_detections, delta_t, min_stat)
        else:
            # For Simple time_difference clustering:
            clusters = cluster_detections_dt(list_detections, delta_t, min_stat) 
        ...
        if clusters:
            ...
            for i_cl, cluster in enumerate(clusters):
                fig_net = plot_multi_stations(detections_for_date, date_key, cluster, i_cl,
                                     path_dat, station_list, dir_stat_name, format, chn=channel)
                if save_result:
