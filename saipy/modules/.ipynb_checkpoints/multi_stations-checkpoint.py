...
def build_pre_clusters(detections, delta_t):
    ...
    clusters = [c for c in clusters if len(set(d['station'] for d in c)) > 1] # at least two stations
    
    return clusters

# Time-difference & pattern clustering _________________________________________________________________
def cluster_detections(detections, delta_t):
    """
    ...
    """
    pre_clusters = build_pre_clusters(detections, delta_t)
...
def cluster_detections_dt(detections, delta_t):
    """
